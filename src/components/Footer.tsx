"use client";

import Link from "next/link";
import { useLang } from "@/context/LanguageContext";

const socialLinks = [
  {
    name: "Facebook",
    href: "https://facebook.com/medswiftexpress",
    icon: (
      <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
        <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
      </svg>
    ),
  },
  {
    name: "Instagram",
    href: "https://instagram.com/medswiftexpress",
    icon: (
      <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
        <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z" />
      </svg>
    ),
  },
  {
    name: "X (Twitter)",
    href: "https://x.com/medswiftexpress",
    icon: (
      <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
        <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
      </svg>
    ),
  },
  {
    name: "LinkedIn",
    href: "https://linkedin.com/company/medswiftexpress",
    icon: (
      <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
        <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
      </svg>
    ),
  },
];

export default function Footer() {
  const { tx } = useLang();

  const mainLinks = [
    { href: "/", label: tx.nav.home },
    { href: "/about/", label: tx.nav.about },
    { href: "/contact/", label: tx.nav.contact },
  ];


  return (
    <footer className="bg-brand-navy text-white">
      {/* Red accent top bar */}
      <div className="h-1 bg-gradient-to-r from-brand-red via-red-400 to-brand-red" />

      <div className="container-lg py-14">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">

          {/* ── Column 1: Brand + Social ── */}
          <div className="lg:col-span-1">
            <Link href="/" className="inline-block mb-4">
              <img
                src="/logos/logo-white.svg"
                alt="MEDSWIFT EXPRESS"
                className="h-12 w-auto"
              />
            </Link>

            <p className="text-slate-400 text-sm leading-relaxed mb-5 max-w-xs">
              {tx.footer.tagline}
            </p>

            <p className="text-brand-red text-xs font-semibold mb-5">
              📍 {tx.footer.serving}
            </p>

            {/* Social Media Icons */}
            <div>
              <p className="text-slate-400 text-xs font-semibold uppercase tracking-wider mb-3">
                Follow Us
              </p>
              <div className="flex gap-2">
                {socialLinks.map((s) => (
                  <a
                    key={s.name}
                    href={s.href}
                    target="_blank"
                    rel="noopener noreferrer"
                    aria-label={s.name}
                    className="w-9 h-9 bg-white/10 hover:bg-brand-red rounded-xl flex items-center justify-center text-slate-300 hover:text-white transition-all duration-200"
                  >
                    {s.icon}
                  </a>
                ))}
              </div>
            </div>
          </div>

          {/* ── Column 2: Navigation ── */}
          <div>
            <h3 className="font-semibold text-xs uppercase tracking-wider text-slate-400 mb-4">
              {tx.footer.links}
            </h3>
            <ul className="space-y-2.5">
              {mainLinks.map(({ href, label }) => (
                <li key={href}>
                  <Link
                    href={href}
                    className="text-slate-300 hover:text-white text-sm transition-colors duration-150 hover:pl-1"
                    style={{ display: "inline-block", transition: "all 0.15s" }}
                  >
                    {label}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* ── Column 3: Contact + CTA ── */}
          <div>
            <h3 className="font-semibold text-xs uppercase tracking-wider text-slate-400 mb-4">
              {tx.footer.contact}
            </h3>
            <ul className="space-y-2 text-sm text-slate-400 mb-8">
              <li>
                <a href="tel:7579408468" className="hover:text-white transition-colors">
                  📞 (757) 940-8468
                </a>
              </li>
              <li>
                <a href="mailto:beatricenatasha99@gmail.com" className="hover:text-white transition-colors break-all">
                  ✉️ beatricenatasha99@gmail.com
                </a>
              </li>
              <li className="text-slate-500">🕐 Mon–Fri 8am–6pm · Sat 9am–2pm</li>
            </ul>

            <div className="p-4 bg-white/5 rounded-2xl border border-white/10">
              <p className="text-slate-300 text-xs mb-3 leading-relaxed">
                Ready to get started with MEDSWIFT EXPRESS?
              </p>
              <Link
                href="/contact/"
                className="inline-flex items-center gap-1.5 bg-brand-red text-white text-xs font-semibold px-4 py-2.5 rounded-lg hover:bg-brand-redmid transition-colors w-full justify-center"
              >
                {tx.nav.contact} →
              </Link>
            </div>
          </div>

        </div>
      </div>

      {/* Bottom bar */}
      <div className="border-t border-white/10">
        <div className="container-lg py-5 flex flex-col sm:flex-row justify-between items-center gap-3">
          <p className="text-slate-500 text-xs">{tx.footer.legal}</p>
          <Link href="/sitemap.xml" className="text-slate-500 hover:text-slate-300 text-xs transition-colors">Sitemap</Link>
        </div>
      </div>
    </footer>
  );
}
